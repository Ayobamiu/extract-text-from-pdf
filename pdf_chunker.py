    ...
    def merge_results(self, chunk_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for chunk_result in chunk_results:
                ...
                # Merge raw_text
                chunk_raw_text = data.get("raw_text", "")
                if chunk_raw_text:
                    merged_raw_text.append(chunk_raw_text)
                ...
                if structured_data:
                    ...
                    # Merge form_fields
                    form_fields = structured_data.get("form_fields", [])
                    merged_form_fields.extend(form_fields)
            ...
            merged_result = {
                ...
                "raw_text": "\n\n".join(merged_raw_text) if merged_raw_text else "",
                "structured_data": {
            ...
            logger.info(
                ...
                f"{len(merged_markdown)} markdown chunks, {len(merged_raw_text)} raw_text chunks, "
                f"{len(merged_entities)} entities, {len(merged_form_fields)} form fields"
            )
